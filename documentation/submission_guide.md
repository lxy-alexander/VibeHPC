...
2. Run all benchmarks and scenarios you would like to submit results in. **Each benchmark has specific execution requirements** - refer to the README in `code/<benchmark>/` for detailed instructions.

    **Important**: Most LLM benchmarks require a two-step process:
    ```bash
    ...
    # Step 2: Run the harness (after server is ready)
    make run_harness RUN_ARGS="--benchmarks=<BENCHMARK> --scenarios=<SCENARIO> --core_type=trtllm_endpoint"
    ```

    For the high accuracy target (99.9%), add `--accuracy_target=.999` to the harness command.

3. Run the accuracy tests by adding the `test_mode` flag:

    ```bash
    make run_harness RUN_ARGS="--benchmarks=... --scenarios=... --test_mode=AccuracyOnly"
    make run_harness RUN_ARGS="--benchmarks=... --scenarios=... --accuracy_target=.999 --test_mode=AccuracyOnly"
    ```
...
$ make run_audit_harness RUN_ARGS="--benchmarks=... --scenarios=... --accuracy_target=.999"
```
    1. **Do not** run accuracy mode for audit tests.

3. The stdout logs will show `TEST PASS` or `TEST FAIL`.

    1. Some benchmarks may fail the first 2 parts of TEST01 due to nondeterministic CUDA kernel selection at runtime. This causes the raw output value to be slightly different, but does not affect the overall accuracy. The `run_audit_harness` make target you ran in step (2) automatically does the "fallback" manual path described in [TEST01 Part III](https://github.com/mlcommons/inference/tree/master/compliance/nvidia/TEST01#part-iii), so **you do not have to do this by hand**.
