...
# Add visual_gen examples/common to path for shared utilities
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
_VISUAL_GEN_EXAMPLES_COMMON = os.path.join(_SCRIPT_DIR, "../../../../3rdparty/trtllm/tensorrt_llm/visual_gen/examples/common")
if os.path.isdir(_VISUAL_GEN_EXAMPLES_COMMON):
    sys.path.insert(0, _VISUAL_GEN_EXAMPLES_COMMON)
...
from visual_gen.models.transformers.wan_transformer import WanTransformer3DModel
from visual_gen.models.vaes.wan_vae import ditWanAutoencoderKL
from visual_gen.pipelines.wan_pipeline import WanPipeline

from visual_gen.utils.load_export_vfly import export_vfly_checkpoint, load_vfly_checkpoint
...
def load_and_setup_pipeline(args):
    ...
    if args.load_vfly_dit:
        transformer = load_vfly_checkpoint(WanTransformer3DModel, args.vfly_ckpt_path, torch_dtype=torch.bfloat16)
    else:
    ...
    pipe = WanPipeline.from_pretrained(
        args.model_path, vae=vae, transformer=transformer, torch_dtype=torch.bfloat16, **vfly_configs
    )
...
def run_inference(pipe, args, enable_autotuner: bool = False):
    ...
    # Load fixed latent if provided
    fixed_latent = None
    if args.fixed_latent is not None:
        logger.info(f"Loading fixed latent from {args.fixed_latent}")
        fixed_latent = torch.load(args.fixed_latent, map_location="cpu")
...
def main():
    ...
    # Add Wan-specific argument for fixed latent
    parser.parser.add_argument(
        "--fixed_latent", type=str, default=None,
        help="Path to fixed latent .pt file to use instead of random noise"
    )
    ...
    if args.linear_type == "auto" or args.attn_type == "auto":
        ...
        if args.enable_visual_gen_cpu_offload:
            logger.warning("Disable visual_gen cpu offload when using autotuner")
            args.enable_visual_gen_cpu_offload = False

    if args.export_vfly_dit:
        ...
        export_vfly_checkpoint(transformer, args.vfly_ckpt_path)
        return
