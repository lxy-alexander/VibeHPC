...
from code.common.paths import TRTLLM_DIR
...
G_OPENORCA_VALSET_PATH = "build/preprocessed_data/llama2-70b/open_orca_gpt4_tokenized_llama.sampled_24576.pkl"
...
class TRTTester:
    ...
    def create_trt_engine(self):
        ...
        builder_path = TRTLLM_DIR / "examples/llama2/build.py"
        if not builder_path.exists():
            raise RuntimeError(f"TRTLLM not found under {TRTLLM_DIR}, please run git submodule update --init --recursive")
        build_cmd = [
            "python",
            str(TRTLLM_DIR / "examples/llama2/build.py"),
            "--dtype=float16",
            "--log_level=verbose",
            "--enable_context_fmha",
