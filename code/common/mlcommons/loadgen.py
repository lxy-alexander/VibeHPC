...
# Accuracy sample count map - used for AccuracyOnly mode when benchmarks have different
# dataset sizes for accuracy vs performance (e.g., gpt-oss-120b: 4395 acc vs 6396 perf)
accuracy_qsl_size_map = model_config.get("accuracy-sample-count", {}).copy()
...
# gpt-oss-120b is in submission_checker; add underscore alias
benchmark_qsl_size_map["gpt_oss_120b"] = benchmark_qsl_size_map["gpt-oss-120b"]

# WAN22-A14B T2V benchmark (text-to-video)
# Note: full_benchmark_name is "wan-2.2-t2v-a14b" (from submission_benchmark_name())
benchmark_qsl_size_map["wan-2.2-t2v-a14b"] = benchmark_qsl_size_map["wan22-a14b"]= 248  # Number of prompts for T2V benchmark
...
@bind(lg_fields.accuracy_sample_count_override)
...
class UserConf:
    ...
    Attributes:
        ...
        accuracy_sample_count_override: Override for accuracy sample count
    ...
    accuracy_sample_count_override: Optional[int] = None
    ...
    def export(self, path: Path):
        ...
        with path.open(mode='w') as f:
            for field in dcls.fields(self):
                ...
                if field.name in {"test_run", "test_mode"}:
                    continue
...
class LoadgenSettings:
    ...
    def export_user_conf(self):
        ...
        """
        # Use accuracy sample count for AccuracyOnly mode if available
        # (some benchmarks like gpt-oss-120b have different dataset sizes for acc vs perf)
        if self.test_mode == "AccuracyOnly" and self.full_benchmark_name in accuracy_qsl_size_map:
            qsl_size = accuracy_qsl_size_map[self.full_benchmark_name]
        else:
            qsl_size = benchmark_qsl_size_map[self.full_benchmark_name]

        user_conf = scenario_settings(self.scenario)(qsl_size)
    ...
    def export_system_json(self):
        ...
        if isinstance(self.precision, C.Precision):
            ...
            for k, v in self.precision.items():
                ...
                if self.benchmark == C.Benchmark.SDXL and k == "weight_data_types":
                    precision[k] = v['unet'].valstr
                else:
        ...
        if "Triton_CPU" in self.system_name:
            ...
            starting_weights_filename_map = {
                ...
                C.Benchmark.GPT_OSS_120B: "Original Huggingface model weights",
                C.Benchmark.SDXL: "Huggingface model weights hosted by MLCommons",
                C.Benchmark.WAN22_A14B: "Original Huggingface model weights (Wan-AI/Wan2.2-T2V-A14B-Diffusers)",
                C.Benchmark.WHISPER: "Original Huggingface model weights",
                C.Benchmark.Q3VL: "Huggingface model weights hosted by MLCommons (nvidia/Qwen3-VL-235B-A22B-Instruct-NVFP4-MLPerf-Inference-Closed-V6.0)",
            ...
            weight_transformations_map = {
                ...
                C.Benchmark.GPT_OSS_120B: "quantization, affine fusion",
                C.Benchmark.SDXL: "quantization, affine fusion",
                C.Benchmark.WAN22_A14B: "quantization, affine fusion",
                C.Benchmark.RGAT: "none",
                C.Benchmark.WHISPER: "quantization, affine fusion",
                C.Benchmark.Q3VL: "quantization",
                C.Benchmark.RGAT: "none"
            }
        ...
        # v6.0 format: use measurements.json instead of <system>_<scenario>.json
        with (self.directory / "measurements.json").open(mode='w') as f:
