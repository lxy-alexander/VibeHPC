...
from code.common.paths import MLCOMMONS_INF_REPO
...
def generate_measurements_entry(system_name, short_benchmark_name, full_benchmark_name, scenario, input_dtype, precision, flag_dict):
    ...
    As of v6.0, measurements files (measurements.json, mlperf.conf, user.conf, README.md) are placed
    directly at results/<system>/<benchmark>/<scenario>/ level instead of a separate measurements/ directory.
    During staging, these files are copied into the results directory structure.
    """
    ...
    # Generate measurements.json (v6.0 format - simplified name instead of <system>_<scenario>.json)
    measurements_json_path = os.path.join(measurements_dir, "measurements.json")
    generate_measurements_json(system_name, measurements_json_path, short_benchmark_name, input_dtype, precision)
...
def generate_mlperf_conf(mlperf_conf_path):
    shutil.copyfile(MLCOMMONS_INF_REPO / "mlperf.conf", mlperf_conf_path)
...
def generate_measurements_json(system_name, measurements_json_path, short_benchmark_name, input_dtype, precision):
    """Generate measurements.json file for MLPerf submission (v6.0 format).

    This is the new format that replaces the old <system>_<scenario>.json file.
    The file contains metadata about input data types, weights, and transformations.
    """
    _generate_measurement_data(system_name, measurements_json_path, short_benchmark_name, input_dtype, precision)
...
def generate_system_json(system_name, system_json_path, short_benchmark_name, input_dtype, precision):
    ...
    Deprecated: Use generate_measurements_json instead.
    """
    _generate_measurement_data(system_name, system_json_path, short_benchmark_name, input_dtype, precision)


def _generate_measurement_data(system_name, output_path, short_benchmark_name, input_dtype, precision):
    """Internal function to generate measurement data JSON file."""
