...
class TRTLLMLibraryLoader:
    ...
    lib_path: os.PathLike = paths.TRTLLM_DIR
    tp_size: int = 1
    pp_size: int = 1
    ...
    def __call__(self,
        ...
        # Check ENV environment variable to determine if we should use the -I flag
        # ENV=release: tensorrt_llm is pre-installed in the container, use -I to avoid conflicts with local dirs
        # ENV=dev (or unset): tensorrt_llm is in local directory, don't use -I as it needs cwd in sys.path
        if os.environ.get('ENV', 'dev').lower() == 'release':
            cmd = [python_bin, '-I', main_path] + argv
        else:
...
class TRTLLMQuantizerOp(Operation):
    ...
    def run(self, scratch_space, dependency_outputs):
        script_location = self.trtllm_loader.lib_path / self.script_path
...
class TRTLLMBuilderOp(Operation):
    ...
    def run(self, scratch_space, dependency_outputs):
        ...
        assert (ckpt_path / 'config.json').exists(), f"config.json not found at: {ckpt_path}, is this a valid model checkpoint?"
