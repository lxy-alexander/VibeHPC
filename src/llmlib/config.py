...
class TrtllmExtraYAMLConfig(TrtllmHarnessConfig):
    ...
    enable_metrics: bool = True  # enable /metrics and /perf_metrics endpoints

    # these are auto-configured in post_init
    extra_config_yaml: str = None

    DEFAULT_EXTRA_CONFIG = {
        # toggle using harness_fields.enable_metrics
        'print_iter_log': True,
        'enable_iter_perf_stats': True,  # enable /metrics endpoint for trtllm-serve
        'return_perf_metrics': True, # enable /perf_metrics endpoint for trtllm-serve
    ...
    DEFAULT_RUNTIME_FLAGS = {
        ...
        'sampler_type': None,  # can be 'TRTLLMSampler', 'DefaultSampler', etc.

        # can be ["CUTLASS", "CUTEDSL", "WIDEEP", "TRTLLM", "VANILLA"]
        'moe_backend': 'CUTLASS',
        'use_low_precision_moe_combine': None,  # for moe_config
        ...
        'enable_lm_head_tp_in_adp': None,

        'num_postprocess_workers': None,

        # speculative decoding config (MTP/Eagle)
        # set 'decoding_type' to enable, e.g. 'MTP' or 'Eagle'
        'speculative_decoding': {
            'decoding_type': None,  # e.g. 'MTP', 'Eagle'
            'num_nextn_predict_layers': None,  # for MTP
            'max_draft_len': None,
            'model_dir': None,
            'eagle3_layers_to_capture': None,  # e.g. [-1]
        },
    }
    ...
    def __post_init__(self):
        ...
        self.extra_config_yaml = self._generate_extra_config_yaml(
            self.runtime_flags,
    ...
    def _generate_extra_config_yaml(cls,
        ...
        Args:
            ...
            enable_metrics: Whether to enable metrics' endpoints
        ...
        config_dict |= {
            'backend': runtime_flags['trtllm_backend'],
            'tensor_parallel_size': runtime_flags.get('tensor_parallelism'),
            'pipeline_parallel_size': runtime_flags.get('pipeline_parallelism'),
            'moe_expert_parallel_size': runtime_flags.get('moe_expert_parallelism'),
            'max_batch_size': runtime_flags.get('max_batch_size'),
            'max_num_tokens': runtime_flags.get('max_num_tokens'),
            'max_seq_len': build_flags.get('max_seq_len'),
        }
        ...
        if using_pytorch:
            ...
            # NOTE(vir):
            # metrics only supported by pytorch backend
            if enable_metrics:
                config_dict |= {
                    'print_iter_log': True,
            ...
            config_dict['stream_interval'] = runtime_flags.get('stream_interval')
            config_dict['disable_overlap_scheduler'] = runtime_flags.get('disable_overlap_scheduler')
        ...
        config_dict |= {
            ...
            'enable_lm_head_tp_in_adp': runtime_flags.get('enable_lm_head_tp_in_adp'),
            'num_postprocess_workers': runtime_flags.get('num_postprocess_workers'),
        }
        ...
        if using_pytorch:
            ...
            # Add torch_compile_config if specified
            if build_flags.get('torch_compile_config'):
                config_dict['torch_compile_config'] = build_flags['torch_compile_config']

            # Add moe_config if moe_backend is specified
            if runtime_flags.get('moe_backend'):
                moe_config = {
                    'backend': runtime_flags['moe_backend'],
                    'use_low_precision_moe_combine': runtime_flags.get('use_low_precision_moe_combine'),
                }
                config_dict['moe_config'] = moe_config

            # Add enable_lm_head_tp_in_adp if specified
            config_dict['enable_lm_head_tp_in_adp'] = runtime_flags.get('enable_lm_head_tp_in_adp')

            # Add cuda_graph_config if CUDA graphs are enabled
            if runtime_flags['use_cuda_graphs']:
                ...
                config_dict['cuda_graph_config'] = {
                    ...
                    # 'max_batch_size' can be added if needed, based on runtime_flags
                }

            # Add cache_transceiver_config if specified (for disaggregated serving)
            config_dict['cache_transceiver_config'] = {
                'max_tokens_in_buffer': runtime_flags.get('cache_transceiver_max_tokens'),
                'backend': runtime_flags.get('cache_transceiver_backend', 'DEFAULT'),
            }
            ...
            # Add speculative_config if speculative decoding is enabled
            config_dict['speculative_config'] = runtime_flags.get('speculative_decoding', None)

            # Add sampler_type if specified
            config_dict['sampler_type'] = runtime_flags.get('sampler_type')
...
class TrtllmEndpointConfig(TrtllmExtraYAMLConfig):
    ...
    def __post_init__(self):
        ...
        # Skip build/server config derivation if using YAML override
        if self.trtllm_yml_override:
            logging.info(f"Using trtllm_yml_override: {self.trtllm_yml_override} (skipping server config derivation)")
            # Still need max_concurrency for harness - use default or from runtime_flags
            if self.runtime_flags['max_concurrency'] is None:
                # Default to a reasonable value when using override
                self.runtime_flags['max_concurrency'] = self.runtime_flags.get('max_concurrency', 512)
            return
        ...
        if self.runtime_flags['max_concurrency'] is None:
            ...
            if concurrency is not None:
                if self.build_flags['enable_attention_dp'] and self.tensor_parallelism is not None:
                    # since max-bs applies per attention-rank,
                    # when enabled, each TP rank has a DP rank of attention block
                    concurrency *= self.tensor_parallelism
...
class TrtllmDisaggEndpointConfig(TrtllmEndpointConfig):
    ...
    def __post_init__(self):
        ...
        # assure mpirun -n <num_ranks> is feasible (single-node leader mode)
        if self.mpi_mode == MPIMode.LEADER and (single_node_mpi := os.environ.get('SLURM_JOB_NODELIST', False)):
            num_local_gpus = len(DETECTED_SYSTEM.accelerators[GPU])
        ...
        if self.mpi_mode != MPIMode.LEADER and \
