...
#
# Modified version of scaleout/run_scaleout.sh for Dynamo disaggregated serving.
# Changes:
# - Added --workspace option for explicit workspace path override
# - Added per-DP log directories to prevent log file collisions
# - Fixed GPU isolation for intra-node TP workers using --container-env
# - Standardized make command syntax
...
# Helper function to setup per-DP log directories
# Sets dp_log_dir and dp_container_log_dir variables and creates directories
setup_dp_log_dirs() {
    local rank=$1
    dp_log_dir="${HOST_LOG_DIR}/dp_${rank}"
    dp_container_log_dir="${LOG_DIR}/dp_${rank}"
    mkdir -p "${dp_log_dir}/slurm_logs"
}
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  --workspace <path>           Explicit workspace path override"
...
if [[ "$stage" == "server" || "$stage" == "all" ]]; then
    if [[ $num_gpus_per_dp -ge $gpus_per_node ]]; then
        ...
        for ((i=0; i<dp_multiplicity; i++)); do
            ...
            # Create unique log directory per DP instance to avoid log file collisions
            setup_dp_log_dirs $i

            echo "Launching DP rank $i (URL: $server_url, Nodes: $node_list)..."
            server_args="$run_args --trtllm_server_urls=$server_url"
            export DP_RANK=$i
            LOG_DIR="$dp_container_log_dir" \
            run_srun \
                --overlap \
                --output="${dp_log_dir}/slurm_logs/run_llm_server.stdout" \
                --error="${dp_log_dir}/slurm_logs/run_llm_server.stderr" \
                --export=ALL \
                ...
                --mpi=pmix \
                $extra_srun_flags \
                make run_llm_server "RUN_ARGS=$server_args" "LOG_DIR=$dp_container_log_dir" "SYSTEM_NAME=$atomic_system" &
        done
    else
        ...
        for ((j=0; j<dp_per_node; j++)); do
            ...
            for node_name in $allocated_nodes; do
                ...
                # Create unique log directory per DP instance to avoid log file collisions
                setup_dp_log_dirs $rank_idx

                echo "Launching DP rank $rank_idx on $node_name (GPUs: $gpu_list, URL: $server_url, IPC: $ipc_addr)..."
                server_args="$run_args --trtllm_server_urls=$server_url"
                export DP_RANK=$rank_idx
                # For intra-node TP workers with multiple DP per node, use --container-env
                # to pass CUDA_VISIBLE_DEVICES into the container, bypassing SLURM's GPU
                # allocation which overrides --export values.
                if [[ $dp_per_node -gt 1 && $dp_multiplicity -gt 1 ]]; then
                    # Multiple workers on same node - need GPU isolation
                    # Set CUDA_VISIBLE_DEVICES inline and pass via --container-env
                    # This bypasses SLURM's GPU allocation that overrides --export
                    ...
                    run_srun --overlap \
                        ...
                        --error="${dp_log_dir}/slurm_logs/run_llm_server.stderr" \
                        --export=ALL \
                        --container-env=CUDA_VISIBLE_DEVICES,TLLM_SPAWN_PROXY_PROCESS_IPC_ADDR,LOG_DIR \
                        --container-image="$container_image" \
                        --container-mounts="${host_vol}:${container_vol},${mlperf_scratch_path}:/home/mlperf_inference_storage" \
                        --container-workdir="$container_vol" \
                        --container-remap-root \
                    ...
                    # Single worker per node or single DP - standard export works
                    LOG_DIR="$dp_container_log_dir" \
                    run_srun --overlap \
                        --output="${dp_log_dir}/slurm_logs/run_llm_server.stdout" \
                        --error="${dp_log_dir}/slurm_logs/run_llm_server.stderr" \
                        --export=ALL,NVIDIA_VISIBLE_DEVICES="$gpu_list",TLLM_SPAWN_PROXY_PROCESS_IPC_ADDR="$ipc_addr" \
                        --container-image="$container_image" \
                        --container-mounts="${host_vol}:${container_vol},${mlperf_scratch_path}:/home/mlperf_inference_storage" \
                        --container-workdir="$container_vol" \
                        --container-remap-root \
